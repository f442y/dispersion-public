...
import com.github.f442y.dispersion.core.application.PermitConfigAPI;
...
public final class StateMachineCallable<CONTEXT extends StateMachineContext,
    ...
    private static final Logger log = LoggerFactory.getLogger(StateMachineCallable.class);
    ...
    private final PermitConfigAPI permitConfig;
    ...
    public OUTPUT call() throws ExecutionException {
        ...
        StateMachineEventLog stateMachineEventLog = new StateMachineEventLog(stateMachineConfiguration, this);
        ...
        try {
            ...
            while (active.get()) {
                // *Action*
                // get and run the action in the state configuration
            ...
            switch (stateMachineException) {
                // todo: log and report exception, add where exception has occurred
                case ActionException actionException ->
                        log.error("Action exception in state ({}) -> ({})", currentState, actionException.getMessage());
                case TransitionException transitionException -> log.error(
                        "Transition exception in state ({}) -> ({})",
                        currentState,
                        transitionException.getMessage()
                );
            }
            throw new ExecutionException(stateMachineException);
        } finally {
    ...
    public static class StateMachineCallableBuilder<CONTEXT extends StateMachineContext,
        ...
        private PermitConfigAPI permitConfig;
        ...
        public StateMachineCallableBuilder<CONTEXT, STATE_KEY, INPUT, OUTPUT> permitConfig(PermitConfigAPI permitConfig
        ) {
