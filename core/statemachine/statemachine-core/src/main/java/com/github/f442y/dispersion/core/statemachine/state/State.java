...
public abstract non-sealed class State<CONTEXT extends StateMachineContext,
    ...
    public State(@Nonnull Action<CONTEXT, ? extends ActionDependencies<?, ?, ?>> action,
                 @Nonnull Transition<CONTEXT, STATE_KEY, ? extends TransitionStateKey<STATE_KEY>> transition
    ) {
        this.action = action;
