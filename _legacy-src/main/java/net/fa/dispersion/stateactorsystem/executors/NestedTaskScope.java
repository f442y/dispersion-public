...
public class NestedTaskScope<CTX extends StateMachineContext<CTX>,
    ...
    public final void addStateMachinesActorTaskToFork(NestedStateMachine<CTX, PARENT_CTX>[] stateMachines) {
        var stateMachineActors = Arrays.stream(stateMachines)
                                       ...
                                       .map(stateMachine -> {
                                           try (var callableActor =
                                                        new StateMachineCallableActor.StateMachineCallableActorBuilder<CTX,
                                                   Void>().stateMachine(
                                                   stateMachine)) {
                                               return callableActor;
                                           }
                                       })
                                       // each actor (callable) is added to the scope (using '.fork()') creating a
                                       // subtask
                                       // the actor and subtask (converted to future) are wrapped as a
                                       // StateMachineActorWithFuture
                                       .map(stateMachineCallableActor -> new StateMachineActorWithFuture<>(
    ...
    protected void handleComplete(StructuredTaskScope.Subtask<? extends Void> subtask) {
        switch (subtask.state()) {
            case SUCCESS, UNAVAILABLE -> {
                log.trace(
                        "Thread: {} | Nested Scope subtask finished [{}]", Thread.currentThread().threadId(),
                        subtask.state()
                );
            }
            case FAILED -> {
