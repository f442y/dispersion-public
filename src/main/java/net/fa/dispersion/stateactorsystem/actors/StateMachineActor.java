 ...
 */
@Slf4j
public sealed class StateMachineActor<CTX extends StateMachineContext<CTX>, RTN> implements Actor<ActorMessage> permits
        StateMachineCallableActor,
        StateMachineRunnableActor,
        StateMachineSupplierActor {

    private final AtomicBoolean active = new AtomicBoolean(true);
    private final LinkedBlockingQueue<ActorMessage> mailbox = new LinkedBlockingQueue<>();
    ...
    public Actor<ActorMessage> tell(@NonNull final ActorMessage message) {
        switch (message) {
            case ControlActorMessage controlActorMessage -> control(controlActorMessage);
            default -> mailbox.offer(message);
        }
        return this;
    }
            ...
            switch (stateConfig.state()) {
                ...
                case AbstractStateMachineState<CTX> stateMachineState -> {
                    try {
                        ...
                        switch (stateMachineState.getAction()) {
                            case WaitingAction<CTX> waitingAction -> {
                                // await a message to be available in the mailbox
                                ActorMessage message = mailbox.take();
                                // push message through to action to run action
                                stateMachineContext = waitingAction.receiveMessage(message, stateMachineContext);
                            }
                            case ProgressingAction<CTX> progressingAction -> {
                                // progress/run action, without blocking/waiting
                                stateMachineContext = progressingAction.progress(stateMachineContext);
                            }
                        }
